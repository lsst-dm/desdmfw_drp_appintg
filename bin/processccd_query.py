...
def query_db(args):
    ...
    if args.visittag is not None:
        sql += " and vt.tag in ('%s') and vt.visit=i.visit and vt.ccd=i.ccd" % (
            "','".join(miscutils.fwsplit(args.visittag, ',')))
...
def main(argv):
    ...
    parser.add_argument(
        '--calib_select', default='filename, filter, ccd, filetype, calib_date', action='store')
    parser.add_argument(
        '--raw_select', default='filename, filter, visit, ccd, pointing, dateobs, field', action='store')
    parser.add_argument('--tract', required=True, action='store', type=int)
    parser.add_argument('--patch', required=False, action='store',
                        help='Must match the patch string format in overlap table')
    parser.add_argument('--filter', required=False, action='store',
                        help="Include only certain filters, comma-separated list")
    parser.add_argument('--visit', required=False, action='store',
                        help="Include only certain visits, comma-separated list")
    parser.add_argument('--ccd', required=False, action='store',
                        help="Include only certain ccds - note doesn't support visit+ccd pairs")
    parser.add_argument('--visittag', required=False, action='store')
    parser.add_argument('--blacklist', required=False, action='store_false',
                        help="Doesn't include if exists in blacklist table")
    parser.add_argument('--blacklist_code', required=False, action='store',
                        help="Use if only want to blacklist for certain codes, comma-separated list")
